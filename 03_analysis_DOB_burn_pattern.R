...
#group and subset data for testing, add attribute for if fire is active
FireBurn_LUT<-FireWeather %>%
  mutate(BurnDay=ifelse(area_Ha>0,wDay,0)) %>%
  dplyr::filter(BurnDay>0) %>%
  group_by(Fire) %>%
  dplyr::summarise(NumDaysBurned=n(), fireStart=min(BurnDay), fireEnd=max(BurnDay))
  

FireDataG<-FireWeather %>%
  group_by(wDay) %>%
  dplyr::summarise(n=n(), area=sum(area_Ha), wind=max(wind_speed), fwi=max(IDW_fwi),
                   isi=max(IDW_isi), wind_direction=first(wind_direction)) #%>%
  #dplyr::filter(wDay>208 & wDay<245)

#plot fwi and isi to see if there is a seasonal pattern
plot(FireDataG$wDay,FireDataG$fwi)

FireDataG$x1<-FireDataG$wDay
FireDataG$y1<-FireDataG$isi
max(FireDataG$fwi)#116
max(FireDataG$wDay)#304

ggplot(data=FireDataG, aes(x=wDay,y=fwi)) +
  geom_line() +
  xlim(min(116), max(304)) +
  ylim(min(0), max(65)) +
  geom_smooth(method=lm, se=FALSE)

ggplot(data=FireDataG, aes(x=wDay,y=isi)) +
  geom_line() +
  xlim(min(116), max(304)) +
  ylim(min(0), max(25)) +
  geom_smooth(method=lm, se=FALSE)
...
FireCheck<-FireDataG %>%
  dplyr::filter(wDay<209)


ggplot(data=FireDataG, aes(x=y1)) +
  geom_histogram()
